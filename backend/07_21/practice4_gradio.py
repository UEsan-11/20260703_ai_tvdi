...
啟動方式：
    uv run backend/07_21/practice4_gradio.py
"""
...
def _build_chart(df: pd.DataFrame, symbol: str, period_label: str):
    ...
    bars = ax_volume.bar(x, volume, width=0.8, alpha=0.22, color="#0288d1")
    line = ax_price.plot(x, close, color="#1565c0", linewidth=2.2, marker="o", markersize=3.8)

    ax_price.set_title(f"{symbol} 期間：{period_label}", fontsize=14, pad=10)
    ax_price.set_ylabel("收盤價", color="#1565c0")
    ax_volume.set_ylabel("成交量", color="#0288d1")
...
.stock-btn {
  background: linear-gradient(120deg, var(--accent), #0288d1) !important;
  border: none !important;
  color: #fff !important;
  font-weight: 700 !important;
  box-shadow: 0 10px 24px rgba(25, 118, 210, 0.32);
}
